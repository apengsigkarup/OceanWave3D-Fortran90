...
!<
SUBROUTINE funPressureTerm_spk(t,g,Nx,Ny,FineGrid,Wavefield)
  USE Precision
  USE Constants
  USE DataTypes
  USE pdamp_csr
  ...
  If (PDampingOnOff /= 0) Then
     id=1
     IF(PDampZones_csr(id)%type==0) THEN
        !
        ...
        Do i=1,Nx
           k=(j-1)*Nx+i
           Wavefield%NuD(i,j)=-PDampZones_csr(id)%gamEta(k)*Wavefield%E(i,j)
        END Do
        !
     ...
     ELSE
        ...
        Do j=1,Ny
           Do i=1,Nx
              k=(j-1)*Nx+i
              Wavefield%NuD(i,j)=-PDampZones_csr(id)%gamEta(k)*Wavefield%E(i,j)
              Wavefield%Pd(i,j)=-PDampZones_csr(id)%gamPhi(k)*Wavefield%P(i,j)
           END Do
        END Do
     END IF
  END If
END SUBROUTINE funPressureTerm_spk
