subroutine incident_nonlinear_wf_finite_standing(wf_type, Wavefield, & !ramp_type,&
...
IMPLICIT NONE
INTERFACE
	SUBROUTINE phi_nonlinear_3D_standing(H,angle,Lx,Ly,x,y,z,d,eta,omega,time,grav,phix,phiy,phiz,phi,phit)
    USE PRECISION
   	REAL(KIND=long), INTENT(IN)  :: H,Lx,Ly,x,y,z,d,omega,time,grav,angle,eta
...
wavenum = SQRT((two*pi/(two*SFSOL%L))**2+(zero*two*pi/SFSOL%L)**2)
omega = sqrt(grav*wavenum*tanh(wavenum*d))
omega=wf_type/abs(wf_type)*omega
!
SFsol%HH = 0.08934064760240
SFSOL%k=SQRT((two*pi/(two*SFSOL%L))**2+(zero*two*pi/SFSOL%L)**2)
...
SFSOL%c=SQRT(grav/SFSOL%k*TANH(SFSOL%k*d))
      ...
      DO j=1,nfx
        DO k=1,nfy
            CALL eta_nonlinear_3D_standing(H,angle,SFsol%L,SFsol%L,xp(j,k),yp(j,k),omega,tp,Wavefield%E_I(j,k), &
            	 Wavefield%Ex_I(j,k),Wavefield%Ey_I(j,k),Wavefield%Exx_I(j,k),Wavefield%Eyy_I(j,k),Wavefield%Et_I(j,k))
        	!
        	! Here phi, is computed on the free-surface z=eta+eta^I
        	!
            zbp = Wavefield%E(j,k)+FAC*Wavefield%E_I(j,k)
        	CALL phi_nonlinear_3D_standing(H,angle,SFsol%L,SFsol%L,xp(j,k),yp(j,k),zbp,d,Wavefield%E(j,k)+FAC*Wavefield%E_I(j,k),&
            	omega,tp,grav,Wavefield%Px_I_s(j,k),Wavefield%Py_I_s(j,k),Wavefield%Pz_I_s(j,k),Wavefield%P_I_s(j,k),&
                Wavefield%Pt_I_s(j,k))
        ENDDO
      END DO
      !
	!
    ...
    DO j = 1+GhostGridY, nfy-GhostGridY
		DO i = 1+GhostGridX, nfx-GhostGridX
            ...
            zbp=((bottom(i,j)+Wavefield%E(i,j)+FAC*Wavefield%E_I(i,j))*zp(1+GhostGridZ)-bottom(i,j))
            !zbp=d*zp(2)-d
            CALL phi_nonlinear_3D_standing(FAC*H,angle,SFsol%L,SFsol%L,xp(i,j),yp(i,j),zbp,d,&
            	Wavefield%E(i,j)+FAC*Wavefield%E_I(i,j),omega,tp,grav,Wavefield%Px_I_bp(Gidx3),&
    ...
    DO j = 1+GhostGridY, nfy-GhostGridY
        DO k = 1, nfz
            ...
            ELSE
                ...
                zbp=((bottom(i+GhostGridX,j)+Wavefield%E(i+GhostGridX,j)+FAC*Wavefield%E_I(i+GhostGridX,j))*zp(k)&
                	-bottom(i+GhostGridX,j))
                !zbp=d*zp(k)-d
                CALL phi_nonlinear_3D_standing(FAC*H,angle,SFsol%L,SFsol%L,xp(i+GhostGridX,j),yp(i+GhostGridX,j),zbp,d,&
                	Wavefield%E(i+GhostGridX,j)+FAC*Wavefield%E_I(i+GhostGridX,j),omega,tp,grav, &
    ...
    DO i = 1+GhostGridX, nfx-GhostGridX
        ...
        DO k = 1, nfz
            ...
            ELSE
                ...
                zbp=((bottom(i,j+GhostGridY)+Wavefield%E(i,j+GhostGridY)+FAC*Wavefield%E_I(i,j+GhostGridY))*zp(k) &
                	-bottom(i,j+GhostGridY))
                !zbp=d*zp(k)-d
                CALL phi_nonlinear_3D_standing(FAC*H,angle,SFsol%L,SFsol%L,xp(i,j+GhostGridY),yp(i,j+GhostGridY),zbp,d,&
                	Wavefield%E(i,j+GhostGridY)+FAC*Wavefield%E_I(i,j+GhostGridY),omega,tp,grav, &
                    Wavefield%Px_I_bp(Gidx3),Wavefield%Py_I_bp(Gidx3),Wavefield%Pz_I_bp(Gidx3))
            ENDIF
            ! Global index for Boundary Points, which option ? for now cf NormalX,Y,Z
            Wavefield%GidxTableBP(k,i,j) = Gidx3
...
RETURN
END SUBROUTINE incident_nonlinear_wf_finite_standing
