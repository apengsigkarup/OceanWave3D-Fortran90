SUBROUTINE PreProcessDiffStencils(FineGrid,DiffStencils,GhostGridX,GhostGridY,&
GhostGridZ,alpha,beta,gamma)
...
IF (GhostGridZ==0) THEN; Nzp = Nz; ELSE; Nzp = Nz+1; ENDIF
  ...
  DO order=1,2
    DO i = 1, Nzp
      IF (i-1<gamma) THEN
        Diff = gamma-i+1
        CALL TaylorFDStencils1DArbitrary(gamma-Diff,gamma+Diff,order,Stencil,&
        FineGrid%z(i+idx+Diff))
      ELSEIF (i>=Nz-gamma+1) THEN
        Diff = i-(Nzp-gamma)
        CALL TaylorFDStencils1DArbitrary(gamma+Diff,gamma-Diff,order,Stencil,&
        FineGrid%z(i+idx-Diff))
      ELSE
        CALL TaylorFDStencils1DArbitrary(gamma,gamma,order,Stencil,&
        FineGrid%z(i+idx))
      ENDIF
