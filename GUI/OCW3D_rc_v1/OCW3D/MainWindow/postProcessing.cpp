...
void MainWindow::convertTo(){
...
#if HAWC>0
    if (output==4){
        HAWC *hawcClass = new HAWC(convertFiles,this);

        hawcClass->show();
         ui->convertStatus->setText("Done; Input file to HAWC is saved in working directory");
    ...
    if (output==4){
        ...
        flex5 *flex5Class = new flex5(convertFiles,this);

        flex5Class->show();
        ui->convertStatus->setText("Done; Input file to FLEX5 is saved in working directory");
    ...
    if (nFiles>ui->tableWidget->rowCount()){
        for (int i =ui->tableWidget->rowCount();i<nFiles;i++){
            ...
            for (int j = 0; j < 6 ; j++) {
                ...
                if (j==4){sp->setValue(tStart);}
                if (j==5){sp->setValue(ui->timeDuration->value());}
            }
